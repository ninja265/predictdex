...
Get the list of supported African countries for filtering markets by geography.
...
**Supported Countries:** Algeria, Angola, Botswana, Cameroon, Côte d'Ivoire, Egypt, Ethiopia, Ghana, Kenya, Morocco, Mozambique, Namibia, Nigeria, Rwanda, Senegal, South Africa, Tanzania, Tunisia, Uganda, Zambia
...
```http
GET /markets?status=open&category=Politics&currency=ZAR&country=ZA&limit=20&offset=0
```
...
| country | string | - | Filter by country code (`ZA`) or slug (`south-africa`) |
